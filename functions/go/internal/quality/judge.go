...
//
// 観点は並べない。並べるとその観点しか見なくなり、想定外の壊れ方が拾えない。
// タイ語・訳文・学習対象語だけ渡して、不自然なら理由を書かせる。
//
// 例外は「文が1文だけで意味をなすか」の2クラスだけ。これは観点を渡さないと
// 落ちない（2026-09-23 実測、プール62本・4回ずつ）:
//
//	×ทัวร์นี้ไปนามาเมื่อกี้（ツアーが田んぼへ行く）
//	  gpt-5.6-luna 0/3 → 追加後 4/4、gpt-5-mini は追加の前後とも 0/4
//
// 訳文・語法の粗さは観点なしでも拾えるので足さない。
package quality
...
文が1文だけで意味をなすかも見る。文法が正しくても次は natural=false。
人にしかできない動作の主語が、場所・乗り物・組織などになっている（×โรงแรมนี้ไปทะเลมาเมื่อวาน）
出来事が何も決まらず、場面を補わないと中身が空になる（×เขาเพิ่งทำครับ）
