...
func TestScoreVocab(t *testing.T) {
	cases := []struct {
		...
		// 期待値は内挿の結果そのもの（ScoreBias は 0）。
		//
		// 1 段目 [1,50] で 0/4 → 推測補正後 0 → 下限に張り付く
		{"完全初心者", []StageResult{{0, 0}}, 0},
		// 1 段目 2/4 → c=(0.5-0.25)/0.75=1/3 → 0 + round(50/3)=17
		{"1段目で半分", []StageResult{{0, 2}}, 17},
		// 1 段目通過、2 段目 [51,150] で 1/4 → c=0 → 50（1 段目の上限）
		{"1段目のみ通過", []StageResult{{0, 4}, {1, 1}}, 50},
		// 3 段目 [151,300] で 2/4 → 150 + round(150/3)=200
		{"3段目で半分", []StageResult{{0, 4}, {1, 4}, {2, 2}}, 200},
		// 全段通過 → 最上段の上限
		{"天井", []StageResult{{0, 4}, {1, 4}, {2, 4}, {3, 4}, {4, 4}, {5, 4}}, 900},
		// 下降して通過 → 下の段の上限を割らない
		{"下降して通過", []StageResult{{2, 1}, {1, 4}}, 150},
	}
	for _, c := range cases {
