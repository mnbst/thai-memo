...
const (
	...
	// TestPassThreshold は次の段へ上がるのに要る正答数。全問正解を求める。
	//
	// 3 問通過にしていた頃は測定値が実際より大きく出た。誤答の選択肢を同じ段の
	// 語の訳から作る以上、消去法と部分再認で実効の推測正答率は 4 択の 0.25 より
	// 高くなり、採点式の前提（TestChanceRate）とずれるため。シミュレーション
	// （4000 試行、pKnow はロジスティック scale 60）の真値に対する誤差:
	//
	//	実効推測率  真値20  80   150  350  700
	//	3 問通過    0.25    -2  -27  -30  -23  -15   ← 前提どおりなら下振れ
	//	3 問通過    0.45   +30  +16  +20  +31  +24
	//	3 問通過    0.55   +71  +63  +70  +84  +48   ← 実効が上がるほど上振れる
	//	4 問通過    0.25    +8  -27  -47  -44  -24
	//	4 問通過    0.55   +21   -8  -22  -15   +4
	//
	// 全問正解は「知らないと通れない」ので実効推測率にほとんど依存しない。
	// 代償は再現性で、知っていても 1 問のうっかりでその段で終わる（p=0.9 の
	// 段通過率は 0.97 → 0.73）。上振れを嫌って控えめ側の散りを取っている。
	TestPassThreshold = TestItemsPerStage
	...
	// しているぶん上振れるため）。TestPassThreshold を全問正解にして上振れの
	// 主因を断ったので、二重に沈めないよう 0 に戻した。上表の「4 問通過」は
	// この 0 での誤差。補正を足すのは、実データで一貫した上振れを測ってから。
	ScoreBias = 0
)
