...
//
// 語ごとの P には一切触れない。測定値は estimated_vocab に直接書くので、
// 正解を既知語として置き直す必要はないし、誤答を書き戻す必要も無い
// （理由は seedVocabTestUvm を落とした経緯として下に残す）。
//
// かつては段階的に 3 種類の種付けをしていて、順に外した。
//
//   - 境界より下の正解に 0.8 … 再受験で estimated_vocab を下げても前回の 0.8 が
//     窓の平均を押し上げ、以後の sync が前回の測定値まで登り直す
//     （1回目300→2回目100 で約60回の sync で 289 へ戻る）
//   - 未登録語の誤答に 0.15 … 窓は 21 ランクなので下方修正には効かず、
//     GetSessionWords の「未登録 or P=0」から外れて key_word に選ばれなくなる
//     だけだった
//   - 既存 doc の誤答を半減 … 下振れた測定が着地先の走査帯を自分で壊す。
//     900 相当の人が 200 と測られると、走査帯は [200,251] に落ちるうえ、
//     その帯の語の P が同じ測定の誤答で半減し、200 に張り付いて動かなくなる。
//     1 つ目を外したのと同型の「測定の誤差が P 経由で固定される」問題
//
// 4 択 1 問は TestChanceRate=0.35 を置くほど信用していない観測なので、それで
// 積み上げた履歴を動かさない。スコアの決定は集約統計（推測補正した内挿）に
// 任せ、語ごとの P は等倍採点のクイズだけが動かす、という分担にする。
