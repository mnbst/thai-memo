                ...
                items: {
                  ...
                  properties: {
                    ...
                    dummy_reasons: {
                      type: SchemaType.ARRAY,
                      items: { type: SchemaType.STRING },
                      description: '3 dummies in choices と正解以外の各単語が不正解である理由。形式: "単語（ローマ字発音 / 日本語の意味）は〜のため不正解" （例: "เร็ว（reo / 速い）は副詞のため名詞位置に代入不可"）',
                    },
                  },
...
- dummy_reasonsには正解以外の3単語それぞれについて「単語（ローマ字発音 / 日本語の意味）は〜のため不正解」の形式で1行ずつ書く（例: "เร็ว（reo / 速い）は副詞のため名詞位置に代入不可"）。

【ダミー選択のNG例と対処法】
NG1 — 主語・人称代名詞の交換
  例文: ___ กินข้าวอยู่ / 正解: ผม
  NG例: ฉัน, คุณ, เขา（どれも主語として成立するため不可）
  OK例: เร็ว（副詞）, มาก（副詞）, สวย（形容詞）

NG2 — 同カテゴリの食べ物・物・場所
  例文: เขาชอบกิน___ / 正解: ข้าว
  NG例: ผัก, เนื้อ, ขนม（どれも「食べる」目的語として成立するため不可）
  OK例: เร็ว（副詞）, บ้าน（場所）, สวย（形容詞）

NG3 — 類義語・言い換え可能な動詞
  例文: เธอ___หนังสือ / 正解: อ่าน
  NG例: ดู（内容によっては成立）, ซื้อ（文脈によっては成立）
  OK例: กิน（食べる）, นอน（寝る）, วิ่ง（走る）
