...
- 節の並置で省略された論理関係（原因・理由・時間・逆接）は日本語訳で補ってよい。補ってよいのは接続詞・助詞・語順のみ。タイ語に明示のない評価・感想・理由・話者の意図・状況を足すのは禁止
...
- ただし前置すれば自然になるわけではない。主題が直後の動詞の主語と誤読される配置は禁止。特に主語省略の心理・知覚動詞（คิด/รู้สึก/นึก等）の前に場所・物を置かない（×ที่นี่ไม่คิดเลยว่าสวยขนาดนี้ →○ไม่คิดเลยว่าที่นี่จะสวยขนาดนี้）。主題と動詞が自然に共起し文全体が一息で流れるか確認し、崩れるなら通常語順にする
...
- 会話はนะ/ดูで柔らかく。複文はเพราะ/เพื่อ/แล้วで接続するが必須ではない。節の並置だけで原因・時間・逆接、または「状況（主題）＋それへのコメント」を表すのも自然（ยังไม่ขึ้นเครื่องเลย ตื่นเต้นจัง／ที่นี่สวยมาก ไม่อยากกลับเลย／ดูหนังเสร็จ ไปกินข้าวกัน）。無理に接続詞で繋がない
- ได้+動詞は経験・機会として使用
