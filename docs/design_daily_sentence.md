...
| `notify_utc_hour` | サーバー（dailyBatch）＋クライアント | 配信対象クエリの絞り込みキー |

#### 配信対象の絞り込み（`notify_utc_hour`）

バッチは毎時起動するため、素直に users を全件走査すると読み取りが 24×N/日 になる。`timezone` と `preferred_generation_hour` から「現地の希望時刻が UTC の何時に当たるか」を導いて非正規化し、`notify_utc_hour == 現在のUTC時` の1クエリで引くことで **N/日** に落としている。

- 算出はオフセットの引き算ではなく、24通りを実際にローカル変換して一致を探す。`+5:30` / `+5:45` のような分単位オフセットでも現地の各時刻はUTCのいずれか1時刻と1対1に対応する（配信が現地10:30になるズレはあるが、これは絞り込み導入以前と同じ挙動）
- DST 春の切り替え日はその現地時刻自体が存在しないため算出不能。その場合は既存値を維持する（＝その日は配信されない。これも従来と同じ）
- 値の維持は `dailyBatch.resetQuota` が担う。既に全ユーザーを走査しているので追加の読み取りが無く、端末の移動・DST・旧クライアントからの設定変更にも1日以内に追従する。クライアントは配信時刻の設定変更時に即時反映のため自分でも書く
- あくまで足切り用で、配信可否の最終判定は従来どおり `should_deliver` 内のローカル時刻比較が行う。値がずれていても誤配信にはならない
...
通知が届いたのに**1本も読まれなかった**配信セットは、次の配信が成功した時点で削除する
（`deliver_daily_sentence.go: purgeUnreadDeliveredSets`）。未読の在庫は常に最新1セットだけになる。

理由:

- クライアントは未取り込みの配信を30日ぶん全部拾って**古い順**に待機列へ積む
  （`daily_sentence_service.dart`）。無視した日数ぶん古い例文が先に出て、いまのレベルに
  合った最新セットが最後尾へ回る
- key_word の選定は `estimated_vocab` と語ごとの P しか見ず、どちらもクイズ回答でしか
  動かない（`internal/uvm/session.go`）。読まずに溜めた期間は帯が固定されるため、溜まった
  セットは同じ帯の似た語ばかりになる。重複するうえ、消化しても語彙スコアが伸びない

判定は `viewed`（`sentence.SupportsViewTracking` の版ゲート付き）。1本でも既読があるセットは
消さない（消化の途中でカーソルが載っている）。`viewed` を持たない doc は既読扱いなので、
既読トラッキング非対応の版へ配信したぶんは対象外になる。

削除は**通知の送信成功後**に行う。送信前に消すと、通知失敗でロールバックしたときに手元から
例文だけが消える。

クライアント側も待機列を1セットまでに揃える（`daily_set_provider.dart: maxPendingSets`）。
サーバーが消せるのは Firestore の doc だけで、既に端末へ取り込まれた待機ぶんは残るため。
溢れたセットは完了として記録して捨てる（例文自体は履歴に残る）。
